...
write.table(importance(rf_cat3_all), "output/field_breeder/importance_field_rf_cat3_all.txt", sep="\t")
```
...
pdf("figures/field_breeder/metabolites_rf_cat3_all_cv_for_publication.pdf", width = 5, height = 5)
...
# Setting c.sd = 1 is similar to the common “1 s.e. rule”, used in the classification trees literature
rf_cat3_all_vs$selected.vars
rf_cat3_all_vs$best.model.nvars
# 24 metabolites

rf_cat3_all_vs$selec.history$Number.Variables
rf_cat3_all_vs$selec.history$OOB
...
pdf("figures/field_breeder/metabolites_rf_cat3_all_vs_for_publication.pdf", width = 5, height = 5)
par(mar=c(4.3, 4.5, 1, 1))
plot(x$selec.history$Number.Variables, x$selec.history$OOB, 
     type = "o", log = "x", ylim = c(0, 0.5), cex.lab = 1.2,
     col = c(rep(1,7), 2, rep(1,10)), 
     pch = c(rep(19,7), 18, rep(19,10)),
     cex = c(rep(1,7), 1.5, rep(1,10)),
...
rownames(rf_cat3_all_imp) <- analytes$Name
rownames(rf_cat3_all_imp)[which(rownames(rf_cat3_all_imp) == "4-Hydroxyphenyl-beta-glucopyranoside")] <- "Arbutin"
# selected variables
...
write.table(rf_cat3_all_imp_vs, "output/field_breeder/importance_field_rf_cat3_all_vs.txt", sep="\t")

pdf("figures/field_breeder/metabolites_rf_cat3_all_imp_for_publication.pdf", width = 5, height = 5)
par(mar=c(4.5, 17, 0.5, 0.5))
barplot(rf_cat3_all_imp_vs[24:1], horiz = T, names = names(rf_cat3_all_imp_vs[24:1]), 
...
table(predict(rf_cat3_all_reduced, all_breeder_data_vs), all_breeder_info$tol_cat3_fve)
  #      low mid high
  # low  139   7    2
  # mid   14 152    7
  # high   5  13  151
...
accuracy_single_trials_full <- func_get_accuracy(model = rf_cat3_all, 
                                            predictors = 1:nrow(analytes), # all metabolites
...
# accuracy for single trials with reduced model
accuracy_single_trials_reduced <- func_get_accuracy(model = rf_cat3_all_reduced, 
                                                    predictors = rf_cat3_all_vs_idx, # subset of metabolites
                                                    trials = breeder_trial_names)
...
# for publication
write.table(accuracy_single_trials, "output/field_breeder/rf_cat3_all_accuracy_single_breeder_trials.txt", sep = "\t", quote = F)
```
...
# boxplot metabolite level ~ tolerance
```{r boxplot metabolite level ~ tolerance}
pdf("figures/field_breeder/metabolite_boxplot_tolerance.pdf")
for(i in 1:ncol(all_data)){
  boxplot(all_data[,i] ~ samplelist_field_joined$tol_cat3_fve, col = heike_palette_3, main = analytes$Name[i])
}
dev.off()

all_data_agg_tol_trial <- aggregate(all_data, by = list(samplelist_field_joined$trial,
                                                        samplelist_field_joined$tol_cat3_fve), median)

all_data_agg_tol <- aggregate(all_data, by = list(samplelist_field_joined$tol_cat3_fve), median)

dim(all_data_agg_tol_trial)
dim(all_data_agg_tol)

heatmap.2(as.matrix(all_data_agg_tol_trial[,3:117])[,rf_cat3_all_vs_idx], 
          scale = "column", trace = "none", 
          Rowv = "none", dendrogram = "column",
          hclustfun = function(x) hclust(x,method = 'complete'))

heatmap.2(as.matrix(all_data_agg_tol[,2:116])[,rf_cat3_all_vs_idx], 
          scale = "column", trace = "none", 
          Rowv = "none", dendrogram = "column",
