...
analytes_sel_exp_sort <- read.table("data/analytes_table_20exp_overlap_select_ordered.txt", 
...
mpi_fgh_2015_factors <- read.table("output/mpi_fgh_2015/factors.txt", 
                                     sep = "\t", header = T, row.names = 1)

mpi_field_2015_factors <- read.table("output/mpi_field_2015/factors.txt", 
                                     sep = "\t", header = T, row.names = 1)

jki_shelter_2015_factors <- read.table("output/jki_shelter_2015/factors.txt", 
                                     sep = "\t", header = T, row.names = 1)

# jki_field_2015_factors <- read.table("output/jki_field_2015/factors.txt", 
#                                      sep = "\t", header = T, row.names = 1)
```
...
```{r load data}
mpi_fgh_2015_values <- read.table("output/mpi_fgh_2015/gmd_values_select_log10.txt", 
                                    sep = "\t", header = T, row.names = 1, check.names = F)

mpi_field_2015_values <- read.table("output/mpi_field_2015/gmd_values_select_log10.txt", 
                                    sep = "\t", header = T, row.names = 1, check.names = F)

jki_shelter_2015_values <- read.table("output/jki_shelter_2015/gmd_values_select_log10.txt", 
                                    sep = "\t", header = T, row.names = 1, check.names = F)

# jki_field_2015_values <- read.table("output/jki_field_2015/gmd_values_select_log10.txt", 
#                                     sep = "\t", header = T, row.names = 1, check.names = F)
```
...
write.table(factors, "output/merge_valdis_2015/factors.txt", sep = "\t")
```
...
pdf("figures/merge_valdis_2015/NAs_hist.pdf")
...
write.table(values_norm, "output/merge_valdis_2015/values_norm.txt", sep="\t")
```


# Look for outliers after normalization of 3 merged data sets
```{r outlier detection}
pdf("figures/merge_valdis_2015/outlier_hist.pdf")
...
pdf("figures/merge_valdis_2015/values_norm_log10_hist.pdf")
...
pdf("figures/merge_valdis_2015/PCA_scoresplots_normalization_effect.pdf")
...
func_pca_plot_sym(pca_none_rnipals, 1, 2, factors, "trial", c(19,17), 
              ...
              legend.text = c("JKI shelter 2015", "MPI FGH 2015", "MPI field 2015"))
...
func_pca_plot(pca_norm_none_rnipals, 1, 3, factors, "genotype_class", 19, 1.5, "bottomright", 1)
func_pca_plot(pca_norm_pareto_rnipals, 1, 2, factors, "genotype_class", 19, 1.5, "bottomleft", 1)
...
pdf("figures/merge_valdis_2015/PCA_scoresplots_genotype_effect.pdf",6, 6)
par(mar=c(4.5, 4.5, 2, 0.5))
...
pdf("figures/merge_valdis_2015/PCA_scoresplots_treatment_effect.pdf",6, 6)
par(mar=c(4.5, 4.5, 2, 0.5))
...
## PCA plots: report
```{r PCA plots: report}
#pdf("figures/merge_valdis_2015/PCA_scoresplots_report.pdf", 12, 6)
png("figures/merge_valdis_2015/PCA_scoresplots_report.png", 4000, 2000, res = 300)
...
pdf("figures/merge_valdis_2015/boxplot_A179012-101.pdf", 9, 6)

boxplot(values_norm[,35] ~ factors$genotype_class, 
        las = 2, col = cols_genotype_report, 
        ylab = "log10-normalisierter Metabolit-Gehalt")

boxplot(values_norm[,35] ~ factors$genotype_name, las = 2)

# sorted boxplot
matrix_df <- data.frame(value = values_norm[, 35], 
...
boxplot(value ~ matrix_df_sorted, data = matrix_df, 
        ...
        main = analytes_sel_exp_sort$Name[35],
...
#pdf("figures/merge_valdis_2015/boxplot_A179012-101_report.pdf", 12, 4)
png("figures/merge_valdis_2015/boxplot_A179012-101_report.png", 5000, 1200, res=300)
...
write.table(res_anova_treatment_genotype, "output/merge_valdis_2015/res_anova_treatment_genotype.txt", sep="\t")
```


## Boxplots of normalized values per analyte using 2 factors
```{r boxplots of normalized values per analyte using 2 factors}
pdf("figures/merge_valdis_2015/boxplot_treatment_genotype.pdf", width=7, height=8)
...
pdf("figures/merge_valdis_2015/boxplot_treatment.pdf")
...
pdf("figures/merge_valdis_2015/boxplot_sorted_per_genotype.pdf", 10, 6)
func_boxplot_sorted2(values_norm, factors$genotype_name, analytes_sel_exp_sort)
dev.off()


pdf("figures/merge_valdis_2015/boxplot_genotype_class.pdf")
func_boxplot_1fac(values_norm, 
                  factors, 
                  "genotype_class", 
                  res_anova_treatment_genotype, 
...
pdf("figures/merge_valdis_2015/boxplot_sorted_per_genotype_class.pdf", 10, 6)
...
```{r save workspace}
save.image("merge_valdis_2015.RData")
sessionInfo()
```
