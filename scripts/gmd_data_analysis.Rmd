...
# select only samples that refer to the wanted genotypes
...
pairs(pca_norm_log10_pareto_rnipals@scores[,1:5], 
      col=factors$genotype_name, pch=19, main="pareto scaling")
...
palette(cols_genotype_factor)
pairs(pca_norm_log10_pareto_rnipals@scores[,1:5], col=factors$genotype_name, pch=19, main="pareto scaling")
pairs(pca_norm_log10_uv_rnipals@scores[,1:5], col=factors$genotype_name, pch=19, main="unit variance scaling")
pairs(pca_norm_log10_vector_rnipals@scores[,1:5], col=factors$genotype_name, pch=19, main="vector scaling")
pairs(pca_norm_log10_none_rnipals@scores[,1:5], col=factors$genotype_name, pch=19, main="no scaling")
...
palette(cols_genotype_factor)
pairs(pca_norm_log10_none_rnipals@scores[,1:5], col=factors$genotype_name, pch=19, main="rnipals")
pairs(pca_norm_log10_none_bpca@scores[,1:5], col=factors$genotype_name, pch=19, main="bpca")
pairs(pca_norm_log10_none_ppca@scores[,1:5], col=factors$genotype_name, pch=19, main="ppca")
pairs(pca_norm_log10_none_svdimpute@scores[,1:5], col=factors$genotype_name, pch=19, main="svdImpute")
...
legend("topright", levels(factors$genotype_name), fill=cols_genotype_factor, cex=0.8)
...
if(length(levels(factors$sample_time))==1){
  res_anova_adj <- func_anova_2fac(values_norm_log10_woOutlier, factors, "treatment", "genotype_name", 0.01, analytes_sel_exp_sort$Name)
  } else {
  res_anova_adj <- func_anova_3fac(values_norm_log10_woOutlier, factors, "treatment", "genotype_name", "sample_time", 0.01, analytes_sel_exp_sort$Name)
  }
...
func_boxplot_1fac(values_norm_log10_woOutlier, factors, "treatment", res_anova_adj, cols_treatment, analytes_sel_exp_sort$Name)
dev.off()


if(length(levels(factors$sample_time)) > 1){
  pdf(paste("figures/", experiment_string, "/boxplot_sample_time.pdf", sep="") )
  func_boxplot_1fac(values_norm_log10_woOutlier, factors, "sample_time", res_anova_adj, cols_sample_time_factor, analytes_sel_exp_sort$Name)
  dev.off()
  }

pdf(paste("figures/", experiment_string, "/boxplot_genotype.pdf", sep="") )
func_boxplot_1fac(values_norm_log10_woOutlier, factors, "genotype_name", res_anova_adj, cols_genotype_factor, analytes_sel_exp_sort$Name)
dev.off()
```
